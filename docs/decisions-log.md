...
## 2026-07-22 — Zero-config Discord routing: real virtual source + automatic adoption

**Status:** Accepted (supersedes 2026-07-21 virtual microphone ADR)
**Context:** The monitor-of-null-sink design failed in real use. Live diagnosis found: (1) the virtual-mic pseudo-entry existed only in the `DEVICES_GET_OUTPUTS` IPC response — the 3 s device poll broadcast raw lists, so the Output dropdown silently lost the selection (user config found reverted to `auto`); (2) sink monitors are second-class inputs and die with Persona, leaving Discord's stream-restore memory pointing at nothing and rebinding to the raw mic; (3) no feedback existed when any hop failed.
**Decision:** Three structural changes. (1) `virtualMic.ts` creates a **real source** (`module-null-sink media.class=Audio/Source/Virtual`) — apps list "Persona Virtual Mic" as a genuine microphone; Persona feeds `input_FL/FR`, listens via `capture_FL/FR` (topology measured live; description requires backslash-escaped spaces to survive pactl's parser). (2) Routing became an explicit **Speakers/Discord toggle** (`routeMode` in config, migration maps the old `selectedOutput=persona_virtual_mic` to it); the pseudo-device is gone, structurally fixing the dropdown race. (3) **Fully automatic adoption** (user's choice): while a voice is active in Discord mode, Persona sets the virtual mic as default source and moves live hardware-mic capture streams onto it via `pactl move-source-output` (excluding monitoring UIs), reverting all of it on Off/speakers/quit. A status-bar chip (green/amber/gray from polling `pactl list source-outputs`) shows what the call actually hears.
**Alternatives rejected:** (1) Fixing only the device-poll bug — leaves monitor fragility and manual Discord setup. (2) Adoption via confirmation prompts — user chose fully automatic. (3) Keeping the sink+monitor with a remap-source on top — extra node for no benefit over the native virtual-source class.
**Consequences:** Verified live: pulse-API capture (`parecord -d persona_virtual_mic` — Discord's exact path) of a Carla chain shows the processed signal; adoption cycle moves a fake call stream on and off cleanly with no residue. Discovered: WirePlumber migrates non-pinned streams on default-source change alone; explicit moves remain for device-pinned streams (Discord after manual device selection). `buildPresetLinks` now takes a `StereoPorts` destination; monitor semantics in Discord mode = listen to the virtual mic. Discord's noise suppression caveat unchanged (documented).
**Amendment (2026-08-11, pre-merge):** First field test broke: releasing adoption on every Off flipped the system default input, which made Discord raise a device-change dialog; dismissing it recreated Discord's capture stream and invalidated the index-based release bookkeeping, leaving Discord stranded on the virtual mic (Off = silence to the call). Fixes: (1) adoption is **sticky per mode** — engaged while in Discord mode regardless of preset, released only on switching to speakers or quit; (2) **Off in Discord mode routes a clean passthrough** into the virtual mic (real voice, not silence; mute belongs to the call app); (3) release is **stateless** — it scans current streams on the virtual mic and moves them home keyed by app name, immune to stream recreation; (4) monitor links are now disconnected before rebuild on preset switches (previously leaked). Second field-test regression: with adoption engaged the system default source IS the virtual mic, so `input: auto` resolved the chain's own input to it — a circular graph with no real microphone (presets silent). Fix: 'auto' resolution never accepts the virtual mic — it falls back to the displaced source (`getSavedDefaultSource()`) or the first hardware input; release likewise never leaves the default stranded on the virtual mic, even after a mid-adoption crash.
